...
class Extractor(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, documents, labels, weights=None):
        ...
        Returns:
            :class`Extractor`
        """
        block_groups = np.array([self.blockifier.blockify(doc) for doc in documents], dtype=object)
    ...
    def get_html_labels_weights(self, data, attribute_idx=0):
        """
    ...
    def _predict_one(self, document, encoding=None, return_blocks=False):
        ...
        try:
            ...
            # auth_feat = self.auth_feat.transform(blocks)
...
class MultiExtractor(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, blockifier=TagCountReadabilityBlockifier,
        ...
        text_tokenizer = joblib.load('extractnet/models/author_text_tokenizer.pkl.gz')
